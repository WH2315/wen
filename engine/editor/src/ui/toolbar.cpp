...
void Toolbar::render() {
    ...
    const std::string play_label = std::string(playing ? icons::kStop : icons::kPlay) + "###PlayStop";
    if (widgets::toggleButton(play_label.c_str(), playing,
                              playing ? "Stop (Ctrl+P)" : "Play (Ctrl+P)",
    ...
    if (widgets::toggleButton(icons::kPause, global_ui_context->game_paused, "Pause",
                              ImVec2(kButtonWidth, 0.0f))) {
    ...
    if (widgets::toggleButton(icons::kStepForward, false, "Step one frame",
                              ImVec2(kButtonWidth, 0.0f))) {
